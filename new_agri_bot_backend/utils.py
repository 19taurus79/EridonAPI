...
from .config import MANAGERS_ID, TELEGRAM_BOT_TOKEN, logger  # Импорт из config.py
    ...
    for user in user_tg_id_list:
        ...
        try:
            if APP_ENV == "production":
                ...
                logger.info(f"Successfully sent message to manager ID: {telegram_id}")
            else:
                # В режиме разработки просто выводим в консоль
                logger.info(f"DEV MODE: Would send to {telegram_id}: '{message_text}'")
        except Exception as e:
            logger.error(f"Failed to send message to manager ID {telegram_id}: {e}")
